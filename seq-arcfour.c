...
void arcfour(BYTE *str, size_t len, BYTE *key, size_t key_length) {
    BYTE state[256], *keyStream;

    keyStream = malloc(len * sizeof(BYTE));
    arcfour_key_setup(state, key, key_length);
    arcfour_generate_stream(state, keyStream, len);
    for (int i = 0; i < len; i++) {
        str[i] = str[i] ^ keyStream[i];
    }
}
