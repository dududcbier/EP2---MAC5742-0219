...
__global__
void blowfish_cuda_encrypt(const BYTE in[], BYTE out[], const BLOWFISH_KEY *keystruct)
{
   WORD l,r,t; //,i;
...
__global__
void blowfish_cuda_decrypt(const BYTE in[], BYTE out[], const BLOWFISH_KEY *keystruct)
{
   WORD l,r,t; //,i;
...
__global__
void blowfish_cuda_key_setup(const BYTE user_key[], BLOWFISH_KEY *keystruct, size_t len)
{
   BYTE block[8];
   int idx,idx2;
...
void blowfish_key_setup(const BYTE user_key[], BLOWFISH_KEY *keystruct, size_t len)
{
   ...
   cudaMalloc(&block, 8 * sizeof (BYTE));

   errorCheck(cudaMemcpy(user_key, data, length * sizeof(BYTE), cudaMemcpyHostToDevice), (char *) "memcpy host to device");
